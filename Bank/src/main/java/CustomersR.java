...
public class CustomersR {
    ...
    public void insert(CustomerE customersE) throws Exception {
        preparedStatement = connection.prepareStatement("INSERT INTO customers (customer_id, first_name, last_name, father_name, national_code, cell_phone, address, city, birth_date) VALUES (?,?,?,?,?,?,?,?,?)");
    ...
    public void update(CustomerE customersE) throws Exception{
        preparedStatement = connection.prepareStatement("UPDATE customers SET first_name = ?, last_name = ?, father_name = ?, national_code = ?, cell_phone = ?, address = ?, city = ?, birth_date = ? WHERE customer_id = ?");
        ...
        preparedStatement.setString(5, customersE.getCellPhone());
        preparedStatement.setString(6, customersE.getAddress());
        preparedStatement.setString(7, customersE.getCity());
        preparedStatement.setString(8, customersE.getDateOfBirth());
        preparedStatement.setLong(9, customersE.getId());
        preparedStatement.executeUpdate();
    }
/*
